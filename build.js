...
function setPieceCounts(_ref, pieceCounts) {
	var size = _ref.size;
	var whoseTurn = _ref.whoseTurn;
	var y = _ref.y;
...
module.exports = function parsePosition(positionString) {
	var whoseTurn = arguments.length <= 1 || arguments[1] === undefined ? 'x' : arguments[1];

	var rows = positionString.trim().split('\n');
	var size = rows.length;

	var _defaultStartingPiece = defaultStartingPiecesByBoardSize(size);

	var startingPieces = _defaultStartingPiece.pieces;
	var startingCapstones = _defaultStartingPiece.capstones;
	...
	var rowStructure = rows.map(function (column) {
		var spaces = column.split('|');
		...
		return spaces.map(function (space) {
			var rowCharacters = space.trim().split('');
			var pieces = rowCharacters.filter(validPiece);
